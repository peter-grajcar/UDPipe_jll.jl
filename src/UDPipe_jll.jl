...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import UDPipe_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("UDPipe_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "UDPipe_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"UDPipe", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "UDPipe_jll", "src", "UDPipe_jll.jl"))
        @info("UDPipe_ll dev'ed out to /Users/petergrajcar/.julia/dev/UDPipe_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("41a5a540-3a0e-51a4-a518-13553378ad37"))
platforms = [Pkg.Artifacts.unpack_platform(e, "UDPipe", artifacts_toml) for e in artifacts["UDPipe"]]
...
if best_platform === nothing
    @debug("Unable to load UDPipe; unsupported platform $(triplet(platform_key_abi()))")
else
